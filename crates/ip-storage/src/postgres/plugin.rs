...
fn plugin_record(row: &sqlx::postgres::PgRow) -> Result<PluginRecord, StorageError> {
    Ok(PluginRecord {
...
fn plugin_rule(row: &sqlx::postgres::PgRow) -> Result<PluginRule, StorageError> {
...
impl PluginStore for PostgresStore {
    ...
    async fn remove_plugin(&self, checksum: &Checksum) -> Result<(), StorageError> {
        let result = sqlx::query("DELETE FROM plugins WHERE checksum = $1")
            .bind(checksum.to_hex())
            .execute(&self.pool)
            .await;
...
impl PluginRuleStore for PostgresStore {
    async fn put_rule(&self, rule: NewPluginRule) -> Result<PluginRule, StorageError> {
        let plugin = sqlx::query("SELECT row_id, kind FROM plugins WHERE checksum = $1")
