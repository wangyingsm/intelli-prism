...
pub(super) async fn insert_tenant(
    ...
    let result =
        sqlx::query("INSERT INTO tenants (id, key, created_at) VALUES (?, ?, ?) RETURNING row_id")
            .bind(new.id.as_str())
...
pub(super) async fn delete_grants_in_tenant(
    ...
    let deleted = sqlx::query(
        "DELETE FROM grants \
         WHERE user_row_id = (SELECT row_id FROM users WHERE id = ?) \
         AND tenant_row_id = (SELECT row_id FROM tenants WHERE id = ?)",
    )
    .bind(user.as_str())
    .bind(tenant.as_str())
    .execute(connection)
...
impl UserCreateTransactional for SqliteStore {
    type Db = sqlx::Sqlite;
...
impl MemberRemoveTransactional for SqliteStore {
    type Db = sqlx::Sqlite;
