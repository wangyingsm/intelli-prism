...
CREATE TABLE plugin_owners (
    plugin_row_id INTEGER NOT NULL REFERENCES plugins (row_id) ON DELETE CASCADE,
    tenant_row_id INTEGER REFERENCES tenants (row_id) ON DELETE CASCADE,
    created_at    INTEGER NOT NULL
);

CREATE UNIQUE INDEX plugin_owners_owner ON plugin_owners (IFNULL(tenant_row_id, 0), plugin_row_id);
...
INSERT INTO plugin_owners (plugin_row_id, tenant_row_id, created_at)
    SELECT DISTINCT plugin_row_id, tenant_row_id, CAST(strftime('%s', 'now') AS INTEGER)
    FROM plugin_rules;
